use spake2::{Ed25519Group, Spake2, Identity, Password};
use chacha20poly1305::{ChaCha20Poly1305, Key, KeyInit};
use chacha20poly1305::aead::{AeadCore, OsRng, Aead};
use std::error::Error;
...
pub fn start_spake2(password: &str, id_a: &str, _id_b: &str) -> Result<(SpakeState, Vec<u8>), Box<dyn Error>> {
...
pub fn finish_spake2(state: SpakeState, inbound_msg: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
    let key = state.spake.finish(inbound_msg).map_err(|e| format!("Spake error: {}", e))?;
    Ok(key)
}
...
pub fn encrypt(key: &[u8; 32], plaintext: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    let ciphertext = cipher.encrypt(&nonce, plaintext).map_err(|e| format!("Encryption failure: {}", e))?;
...
pub fn decrypt(key: &[u8; 32], ciphertext_with_nonce: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    let cipher = ChaCha20Poly1305::new(Key::from_slice(key));
    let plaintext = cipher.decrypt(nonce.into(), ciphertext).map_err(|e| format!("Decryption failure: {}", e))?;
